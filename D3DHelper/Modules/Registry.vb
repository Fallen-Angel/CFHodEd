...
Friend Module Registry
 ...
 ''' <c>True</c> if it exists.
 ''' </returns>
 Private Function SettingType(ByVal ApplicationName As String, ByVal Name As String) As Microsoft.Win32.RegistryValueKind
  Dim k As Microsoft.Win32.RegistryKey
 ...
 ''' </remarks>
 Private Sub LoadRegistrySetting(Of T)(ByVal ApplicationName As String, ByVal Name As String, ByRef Value As T, Optional ByVal ExpectedKeyType As Microsoft.Win32.RegistryValueKind = Microsoft.Win32.RegistryValueKind.Unknown)
  Dim k As Microsoft.Win32.RegistryKey, o As Object
 ...
 : Exit Sub
  ...
  ' Check type of key.
  If k.GetValue(Name) IsNot Nothing Then _
   If (k.GetValueKind(Name) <> Microsoft.Win32.RegistryValueKind.String) AndAlso _
 ...
 ''' </exception>
 Public Sub SavePresentationParameters(ByVal ApplicationName As String, ByVal _PresentationParameters As PresentParameters)
...
: _DeviceType = DeviceType.Hardware
 ...
 Public Function LoadPresentParameters(ByVal ApplicationName As String) As PresentParameters
  ...
  If Not CanCreateDevice(Adapter, _DeviceType, CreateFlags.SoftwareVertexProcessing, _PresentParameters) Then
   Trace.Assert(False, "Warning: Device settings not supported " & _
  ...
  End If ' If Not CanCreateDevice(Adapter, _DeviceType, CreateFlags.SoftwareVertexProcessing, _PresentParameters) Then
 ...
 Public Function SettingsExists(ByVal ApplicationName As String) As Boolean
  ...
  If (Not SettingExists(ApplicationName, REG_ADAPTER)) OrElse _
     (SettingType(ApplicationName, REG_ADAPTER) <> Microsoft.Win32.RegistryValueKind.DWord) Then _
   Return False

  If (Not SettingExists(ApplicationName, REG_DEVICETYPE)) OrElse _
     (SettingType(ApplicationName, REG_ADAPTER) <> Microsoft.Win32.RegistryValueKind.DWord) Then _
   Return False

  If (Not SettingExists(ApplicationName, REG_CREATEFLAGS)) OrElse _
     (SettingType(ApplicationName, REG_ADAPTER) <> Microsoft.Win32.RegistryValueKind.DWord) Then _
   Return False

  ' Check presentation parameters.
  If (Not SettingExists(ApplicationName, REG_WIDTH)) OrElse _
     (SettingType(ApplicationName, REG_ADAPTER) <> Microsoft.Win32.RegistryValueKind.DWord) Then _
   Return False

  If (Not SettingExists(ApplicationName, REG_HEIGHT)) OrElse _
     (SettingType(ApplicationName, REG_ADAPTER) <> Microsoft.Win32.RegistryValueKind.DWord) Then _
   Return False

  If (Not SettingExists(ApplicationName, REG_FORMAT)) OrElse _
     (SettingType(ApplicationName, REG_ADAPTER) <> Microsoft.Win32.RegistryValueKind.DWord) Then _
   Return False

  If (Not SettingExists(ApplicationName, REG_BACKBUFFERCOUNT)) OrElse _
     (SettingType(ApplicationName, REG_ADAPTER) <> Microsoft.Win32.RegistryValueKind.DWord) Then _
   Return False

  If (Not SettingExists(ApplicationName, REG_ENABLEAUTODEPTHSTENCIL)) OrElse _
     (SettingType(ApplicationName, REG_ADAPTER) <> Microsoft.Win32.RegistryValueKind.DWord) Then _
   Return False

  If (Not SettingExists(ApplicationName, REG_DEPTHSTENCILFORMAT)) OrElse _
     (SettingType(ApplicationName, REG_ADAPTER) <> Microsoft.Win32.RegistryValueKind.DWord) Then _
   Return False

  If (Not SettingExists(ApplicationName, REG_REFRESHRATE)) OrElse _
     (SettingType(ApplicationName, REG_ADAPTER) <> Microsoft.Win32.RegistryValueKind.DWord) Then _
   Return False

  If (Not SettingExists(ApplicationName, REG_PRESENTATIONINTERVAL)) OrElse _
     (SettingType(ApplicationName, REG_ADAPTER) <> Microsoft.Win32.RegistryValueKind.DWord) Then _
   Return False

  If (Not SettingExists(ApplicationName, REG_PRESENTATIONFLAG)) OrElse _
     (SettingType(ApplicationName, REG_ADAPTER) <> Microsoft.Win32.RegistryValueKind.DWord) Then _
   Return False

  If (Not SettingExists(ApplicationName, REG_MULTISAMPLETYPE)) OrElse _
     (SettingType(ApplicationName, REG_ADAPTER) <> Microsoft.Win32.RegistryValueKind.DWord) Then _
   Return False

  If (Not SettingExists(ApplicationName, REG_MULTISAMPLEQUALITY)) OrElse _
     (SettingType(ApplicationName, REG_ADAPTER) <> Microsoft.Win32.RegistryValueKind.DWord) Then _
   Return False

  If (Not SettingExists(ApplicationName, REG_WINDOWED)) OrElse _
     (SettingType(ApplicationName, REG_ADAPTER) <> Microsoft.Win32.RegistryValueKind.DWord) Then _
   Return False

  If (Not SettingExists(ApplicationName, REG_FORCENOMULTITHREADEDFLAG)) OrElse _
     (SettingType(ApplicationName, REG_ADAPTER) <> Microsoft.Win32.RegistryValueKind.DWord) Then _
   Return False
 ...
 Public Function DeleteSettings(ByVal ApplicationName As String) As Boolean
  Try
   ' Delete the sub-key.
   Microsoft.Win32.Registry.CurrentUser.DeleteSubKeyTree(REG_SUBKEY_PREFIX & ApplicationName & REG_SUBKEY_POSTFIX)
