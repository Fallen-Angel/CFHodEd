...
Friend Module Validator
 ...
 Public Function ValidateDeviceType(ByVal _DeviceType As DeviceType) As DeviceType
  ...
  If (_DeviceType <> DeviceType.Hardware) AndAlso _
     (_DeviceType <> DeviceType.Reference) AndAlso _
     (_DeviceType <> DeviceType.Software) AndAlso _
 ...
 : DDMX = True
  ...
  ' Now build the flags.
  NewCreateFlags = CType(IIf(FPUP, CreateFlags.FpuPreserve, False), CreateFlags) Or _
                   CType(IIf(MT, CreateFlags.MultiThreaded, False), CreateFlags) Or _
                   CType(IIf(PD, CreateFlags.PureDevice, False), CreateFlags) Or _
                   CType(IIf(SVP, CreateFlags.SoftwareVertexProcessing, False), CreateFlags) Or _
                   CType(IIf(HVP, CreateFlags.HardwareVertexProcessing, False), CreateFlags) Or _
                   CType(IIf(MVP, CreateFlags.MixedVertexProcessing, False), CreateFlags) Or _
                   CType(IIf(DDM, CreateFlags.DisableDriverManagement, False), CreateFlags) Or _
                   CType(IIf(AGD, CreateFlags.AdapterGroupDevice, False), CreateFlags) Or _
                   CType(IIf(DDMX, CreateFlags.DisableDriverManagementEx, False), CreateFlags) Or _
                   CType(IIf(NWC, CreateFlags.NoWindowChanges, False), CreateFlags)

  ' Compare for residue.
  Trace.Assert(NewCreateFlags = _CreateFlags, _
 ...
 ''' This also checks for compatibility on given devices.
 ''' </remarks>
 Public Function ValidateCreateFlags(ByVal Adapter As Integer, ByVal _DeviceType As DeviceType, ByVal _CreateFlags As CreateFlags) As CreateFlags
  ' Check inputs.
  If ValidateAdapter(Adapter) <> Adapter Then _
   Throw New ArgumentException("Invalid 'Adapter'.") _
 : Exit Function
 ...
 : Exit Function
  ...
  If ((_CreateFlags And CreateFlags.PureDevice) = CreateFlags.PureDevice) AndAlso _
 ...
 : _CreateFlags = _CreateFlags And Not CreateFlags.PureDevice
  ...
  If ((_CreateFlags And CreateFlags.HardwareVertexProcessing) = CreateFlags.HardwareVertexProcessing) AndAlso _
 ...
 : _CreateFlags = _CreateFlags And Not CreateFlags.HardwareVertexProcessing
  ...
  If ((_CreateFlags And CreateFlags.MixedVertexProcessing) = CreateFlags.MixedVertexProcessing) AndAlso _
     (Not CheckHardwareVertexProcessing(Adapter, _DeviceType)) Then _
 ...
 Public Function ValidatePresentInterval(ByVal _PresentInterval As PresentInterval) As PresentInterval
  ...
  Select Case _PresentInterval
   ' KNOWN VALUE
   Case PresentInterval.Default, PresentInterval.One, PresentInterval.Two, _
 ...
 Public Function ValidatePresentFlag(ByVal _PresentFlag As PresentFlag) As PresentFlag
  ...
  If ((_PresentFlag And Not (PresentFlag.LockableBackBuffer Or _
                             PresentFlag.DiscardDepthStencil Or _
                             PresentFlag.DeviceClip Or _
                             PresentFlag.Video)) <> PresentFlag.None) Then _
   Trace.Assert(False, "Warning: '_PresentFlag' has unknown bits set.") _
 : Return _PresentFlag And Not (PresentFlag.LockableBackBuffer Or _
                                PresentFlag.DiscardDepthStencil Or _
                                PresentFlag.DeviceClip Or _
                                PresentFlag.Video)
 ...
 Public Function ValidateMultiSampleType(ByVal _MultiSampleType As MultiSampleType) As MultiSampleType
  ' Check for flag.
  If (_MultiSampleType = MultiSampleType.None) OrElse _
     (_MultiSampleType = MultiSampleType.NonMaskable) Then _
   Return _MultiSampleType

  ' Check for sample count.
  If (_MultiSampleType >= MultiSampleType.TwoSamples) AndAlso _
 ...
 : Exit Function
  ...
  With _PresentParameters
   ...
   If .EnableAutoDepthStencil Then
    ...
    If Not FormatIsDepthStencil(.AutoDepthStencilFormat) Then _
      Trace.Assert(False, "Warning: DepthStencil format seems to be invalid: " & .AutoDepthStencilFormat.ToString() & ".") _
   ...
   If Not FormatIsRGB(.BackBufferFormat) Then _
    Trace.Assert(False, "Warning: Backbuffer format seems to be invalid: " & .BackBufferFormat.ToString() & ".") _
  ...
  : .PresentFlag = ValidatePresentFlag(.PresentFlag)
   ...
   If ((.PresentFlag And PresentFlag.DiscardDepthStencil) = PresentFlag.DiscardDepthStencil) AndAlso _
  ...
  : .MultiSampleQuality = 0
   ...
   If ((.PresentFlag And PresentFlag.DiscardDepthStencil) = PresentFlag.DiscardDepthStencil) AndAlso _
      ((.AutoDepthStencilFormat = DepthFormat.D16Lockable) OrElse _
 ...
 ''' </remarks>
 Public Function ValidatePresentParameters(ByVal Adapter As Integer, ByVal _DeviceType As DeviceType, ByVal _PresentParameters As PresentParameters) As PresentParameters
 ...
 : Exit Function
  ...
  : .BackBufferFormat = DispMode.Format
   ...
   If Not CheckDisplayFormat(Adapter, _DeviceType, _
                             Manager.Adapters(Adapter).CurrentDisplayMode.Format, _
  ...
  : _PresentParameters.BackBufferFormat = DispMode.Format
   ...
   If .EnableAutoDepthStencil Then
    ...
    If Not CheckDepthStencilFormat(Adapter, _DeviceType, _
                                   Manager.Adapters(Adapter).CurrentDisplayMode.Format, _
   ...
   If .MultiSample <> MultiSampleType.None Then
    ...
    If Not CheckMultiSampleType(Adapter, _DeviceType, _
                                Manager.Adapters(Adapter).CurrentDisplayMode.Format, _
                                .BackBufferFormat, .AutoDepthStencilFormat, _
 ...
 ''' Also, this modifies the existing object.
 ''' </remarks>
 Public Function ValidatePresentParametersA(ByVal Adapter As Integer, ByVal _DeviceType As DeviceType, ByVal _PresentParameters As PresentParameters) As Boolean
  Dim PP As PresentParameters = CType(_PresentParameters.Clone(), PresentParameters)
 ...
 Private Function PresentParametersEqual(ByVal PP1 As PresentParameters, ByVal PP2 As PresentParameters) As Boolean
  With PP1
   ...
   If (.BackBufferWidth <> PP2.BackBufferWidth) OrElse _
      (.BackBufferHeight <> PP2.BackBufferHeight) OrElse _
      (.BackBufferFormat <> PP2.BackBufferFormat) Then _
    Return False

   ' Check back-buffer count.
   If (.BackBufferCount <> 0) AndAlso _
      (.BackBufferCount <> PP2.BackBufferCount) Then _
    Return False

   ' Check if depth-stencil is enabled.
   If (Not .EnableAutoDepthStencil) AndAlso _
      ((PP2.EnableAutoDepthStencil) AndAlso _
       (PP2.AutoDepthStencilFormat <> DepthFormat.Unknown)) Then _
   Return False

   ' Check depth-stencil.
   If (.EnableAutoDepthStencil) AndAlso _
      (.AutoDepthStencilFormat <> PP2.AutoDepthStencilFormat) Then _
     Return False

   ' Check multi-sampling.
   If (.MultiSample <> PP2.MultiSample) OrElse _
   ...
   ' Check other flags.
   If (.PresentationInterval <> PP2.PresentationInterval) OrElse _
      (.PresentFlag <> PP2.PresentFlag) OrElse _
