...
Public NotInheritable Class D3DConfigurer
 ...
 ''' </exception>
 Public Shared Sub AddMultiSamplingTechnique(ByVal _MultiSampleType As MultiSampleType, ByVal MultiSampleQuality As Integer)
  ' Check inputs.
 ...
 ''' </remarks>
 Public Shared Sub ShowD3DSettingsWindow(Optional ByVal LockWindowed As Microsoft.VisualBasic.TriState = TriState.UseDefault)
 ...
 : Exit Function
  ...
  If m_CreationFlags <> 0 Then
   ...
   If (((m_CreationFlags And CreateFlags.HardwareVertexProcessing) <> 0) OrElse _
       ((m_CreationFlags And CreateFlags.MixedVertexProcessing) <> 0)) AndAlso _
      (Not CheckHardwareVertexProcessing(Adapter, _DeviceType)) Then _
    Trace.TraceWarning("HWTL not supported.") _
  : m_CreationFlags = m_CreationFlags And Not (CreateFlags.HardwareVertexProcessing Or CreateFlags.MixedVertexProcessing) _
 ...
 ''' validation.
 ''' </exception>
 Friend Shared Function BuildPresentParameters(ByVal Adapter As Integer, ByVal _DeviceType As DeviceType, _
                                      Optional ByVal _PresentParamters As PresentParameters = Nothing, _
                                      Optional ByVal Width_OVERRIDE As Integer = 0, _
                                      Optional ByVal Height_OVERRIDE As Integer = 0, _
                                      Optional ByVal BackBufferFormat_OVERRIDE As Format = Format.Unknown, _
                                      Optional ByVal Windowed_OVERRIDE As TriState = TriState.UseDefault) As PresentParameters
  Dim CurrDispMode As DisplayMode, _
      MaxBackBufferCount As Integer, _
      ChosenDisplayMode As Integer = -1

  Dim DisplayModes() As DisplayMode, _
      BackBufferFormats() As Format, _
      BackBufferCounts() As Integer, _
      DepthStencilFormats() As DepthFormat, _
      MultiSamplingA() As MultiSampleType, _
      MultiSamplingB() As Integer, _
 ...
 : Exit Function
  ...
  ' Debug check.
  Trace.Assert(MultiSamplingA.Length = MultiSamplingB.Length, _
               "Internal error", _
               "L(MSA) <> L(MSB)")
  ...
  With _PresentParamters
   ...
   ' --------------------------------
   If ((Width_OVERRIDE <> 0) AndAlso (Height_OVERRIDE <> 0)) AndAlso _
    ...
    Else _
    .FullScreenRefreshRateInHz = GetMaxRefreshRate(Adapter, _
                                                  .BackBufferWidth, _
                                                  .BackBufferHeight, _
                                                   CurrDispMode.Format)

   Else ' If ((Width_OVERRIDE <> 0) AndAlso (Height_OVERRIDE <> 0)) AndAlso (.Windowed OrElse CheckDisplayMode(Adapter, Width_OVERRIDE, Height_OVERRIDE)) Then
    ' ------------------------
    ...
    If .Windowed Then
     ...
     For I As Integer = 0 To DisplayModes.Length - 1
      If (DisplayModes(I).Width <= .BackBufferWidth) AndAlso _
    ...
    Else ' If .Windowed Then
     ...
     For I As Integer = 0 To UBound(DisplayModes)
      ...
      If CheckDisplayMode(Adapter, DisplayModes(I).Width, DisplayModes(I).Height, _
                          CurrDispMode.Format) Then _
       .BackBufferWidth = DisplayModes(I).Width _
     : .BackBufferHeight = DisplayModes(I).Height _
     : .FullScreenRefreshRateInHz = GetMaxRefreshRate(Adapter, _
                                                      DisplayModes(I).Width, _
                                                      DisplayModes(I).Height, _
    ...
    For I As Integer = 0 To UBound(BackBufferFormats)
     ' Check for compatibility.
     If CheckDisplayFormat(Adapter, _DeviceType, CurrDispMode.Format, _
   ...
   End If ' If ((Width_OVERRIDE <> 0) AndAlso (Height_OVERRIDE <> 0)) AndAlso (.Windowed OrElse CheckDisplayMode(Adapter, Width_OVERRIDE, Height_OVERRIDE)) Then
   ...
   For I As Integer = 0 To UBound(DepthStencilFormats)
    ' Check for compatibility.
    If CheckDepthStencilFormat(Adapter, _DeviceType, CurrDispMode.Format, _
                               .BackBufferFormat, DepthStencilFormats(I), _
                               .Windowed) Then _
   ...
   If Not FormatIsDepthLockable(.AutoDepthStencilFormat) Then
    ...
    For I As Integer = 0 To UBound(MultiSamplingA)
     ' Check for compatibility.
     If CheckMultiSampleType(Adapter, _DeviceType, CurrDispMode.Format, _
   ...
   Else ' If Not FormatIsDepthLockable(.AutoDepthStencilFormat) Then
    ' Give a warning.
    Trace.Assert(False, "Warning: Lockable depth buffers are " & _
    ...
    If ChosenDisplayMode <> -1 Then
     For I As Integer = ChosenDisplayMode To UBound(DisplayModes)
      If (.Windowed) Or (CheckDisplayMode(Adapter, DisplayModes(I).Width, DisplayModes(I).Height)) Then
       ...
       Exit For
      End If ' If (.Windowed) Or (CheckDisplayMode(Adapter, DisplayModes(I).Width, DisplayModes(I).Height)) Then
