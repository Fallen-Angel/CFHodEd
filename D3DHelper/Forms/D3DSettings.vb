...
Friend NotInheritable Class D3DSettings
 ...
 ''' </returns>
 Private Function MakeDisplayModeListItem(ByVal Width As Integer, ByVal Height As Integer, ByVal Format As Format) As String
  Return CStr(Width) & _
         "x" & _
         CStr(Height) & _
         "x" & _
         CStr(FormatBitCount(Format)) & _
         " (" & _
         Format.ToString() & _
         ")"
 ...
 ''' Format of the back-buffer.
 ''' </param>
 Private Shared Sub SplitDisplayModeListItem(ByVal DispMode As String, ByRef Width As Integer, ByRef Height As Integer, ByRef Format As Format)
 ...
 Private Function PopulateDisplayModes() As Boolean
  ...
  For I As Integer = 0 To UBound(DisplayFormats)
   For J As Integer = 0 To UBound(DisplayModes)
    ...
    If ((DisplayModes(J).Width <= Screen.PrimaryScreen.Bounds.Width) AndAlso _
        (DisplayModes(J).Height <= Screen.PrimaryScreen.Bounds.Height)) OrElse _
       (Not Windowed) Then
     ' If both display mode and format are available, add it to list.
     If CheckDisplayFormat(Adapter, _DeviceType, CurrDispMode.Format, DisplayFormats(I), Windowed) AndAlso _
       (Windowed OrElse CheckDisplayMode(Adapter, DisplayModes(J).Width, DisplayModes(J).Height)) Then _
      lstDisplayModes.Items.Add(MakeDisplayModeListItem( _
                                                        DisplayModes(J).Width, _
                                                        DisplayModes(J).Height, _
                                                        DisplayFormats(I)) _
                               )

    End If ' If (DisplayModes(J).Width <= Screen.PrimaryScreen.Bounds.Width) AndAlso (DisplayModes(J).Height <= Screen.PrimaryScreen.Bounds.Height) Then
   ...
   If CheckDisplayFormat(Adapter, _DeviceType, CurrDispMode.Format, DisplayFormats(I), Windowed) Then _
    lstDisplayModes.Items.Add(MakeDisplayModeListItem( _
                                                      CurrDispMode.Width, _
                                                      CurrDispMode.Height, _
                                                      DisplayFormats(I)) _
                             )
  ...
  DefaultMode = MakeDisplayModeListItem(CurrDispMode.Width, _
                                        CurrDispMode.Height, _
                                        CurrDispMode.Format)
 ...
 Private Function SaveAllSettings(ByVal ApplicationName As String) As Boolean
  ...
  SplitDisplayModeListItem(CStr(lstDisplayModes.SelectedItem), _
                           Width, _
                           Height, _
                          _Format)

  ' Build the presentation parameters.
  _PresentParameters = D3DConfigurer.BuildPresentParameters(cboAdapter.SelectedIndex, _
                                                     CType(cboDeviceType.Tag, DeviceType), _
                                                     Nothing, _
                                                     Width, _
                                                     Height, _
                                                    _Format, _
                                                     CType(IIf( _
                                                               chkWindowed.Checked, _
                                                               TriState.True, _
                                                               TriState.False _
                                                              ), TriState) _
                                                    )
 ...
 Private Sub LoadAllSettings(ByVal ApplicationName As String)
  ...
  DispMode = MakeDisplayModeListItem(_PresentParameters.BackBufferWidth, _
                                     _PresentParameters.BackBufferHeight, _
 ...
 Private Sub cboAdapter_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cboAdapter.SelectedIndexChanged
 ...
 Private Sub cmdAdapterInfo_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdAdapterInfo.Click
  Dim f As D3DAdapterInfo

  If cboAdapter.SelectedIndex = -1 Then _
   MsgBox("Please select an adapter first!", _
          MsgBoxStyle.Exclamation Or MsgBoxStyle.DefaultButton1 Or MsgBoxStyle.OkOnly, _
 ...
 : Exit Sub
  ...
  ' Make sure all settings are selected.
  If cboDeviceType.SelectedIndex = -1 Then _
   MsgBox("Please select a device type first!", _
          MsgBoxStyle.Exclamation Or MsgBoxStyle.DefaultButton1 Or MsgBoxStyle.OkOnly, _
 ...
 : Exit Sub
  ...
  ' Make sure all settings are selected.
  If lstDisplayModes.SelectedIndex = -1 Then _
   MsgBox("Please select a display mode first!", _
          MsgBoxStyle.Exclamation Or MsgBoxStyle.DefaultButton1 Or MsgBoxStyle.OkOnly, _
 ...
 Private Sub lstDisplayModes_SelectedValueChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles lstDisplayModes.SelectedValueChanged
  cmdApply.Enabled = True
