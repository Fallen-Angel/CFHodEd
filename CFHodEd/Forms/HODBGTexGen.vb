...
Friend NotInheritable Class HODBGTexGen
 ...
 ''' Class contructor.
 ''' </summary>
 Public Sub New(ByVal HOD As HOD, _
       Optional ByVal doubleTone As Boolean = True, _
       Optional ByVal halfOffset As Boolean = True, _
 ...
 Private Function GenerateBGMSTexture(ByVal HOD As HOD) As BasicMesh()
  ...
  Dim highestY As Single = -1.0E+30F, _
   ...
   With out(I).Part(0)
    ...
    For J As Integer = 0 To .PrimitiveGroupCount - 1
     ...
     For K As Integer = 0 To .PrimitiveGroups(J).IndiceCount - 3 Step 3
      ...
      Dim ind1 As Integer = .PrimitiveGroups(J).Indice(K), _
          ind2 As Integer = .PrimitiveGroups(J).Indice(K + 1), _
          ind3 As Integer = .PrimitiveGroups(J).Indice(K + 2)

      ' Get triangle vertices.
      Dim V1 As BasicVertex = .Vertices(ind1), _
          V2 As BasicVertex = .Vertices(ind2), _
      ...
      ' Unwrap centroid.
      Dim UV As New Vector2(0.5F * CSng(Math.Atan2(c.Z, c.X) / Math.PI + 1), _
      ...
      If UV.X < 0.25 Then
       ...
       If (V1.Tex.X > 0.5) OrElse (V2.Tex.X > 0.5) OrElse (V3.Tex.X > 0.5) Then
        ...
        Dim V4 As BasicVertex = V1, _
            V5 As BasicVertex = V2, _
...
#End If
  ...
  With m_D3DManager.Device
   ...
   If m_Meshes.Length <> 0 Then
    ...
    Dim upperRect() As Direct3D.CustomVertex.PositionColored = New Direct3D.CustomVertex.PositionColored() { _
     New Direct3D.CustomVertex.PositionColored(-10, 10, -1, m_ColourTop), _
     New Direct3D.CustomVertex.PositionColored(-10, 0, -1, m_ColourTop), _
     New Direct3D.CustomVertex.PositionColored(10, 10, -1, m_ColourTop), _
     New Direct3D.CustomVertex.PositionColored(10, 0, -1, m_ColourTop) _
    }
    ...
    Dim lowerRect() As Direct3D.CustomVertex.PositionColored = New Direct3D.CustomVertex.PositionColored() { _
     New Direct3D.CustomVertex.PositionColored(-10, 0, -1, m_ColourBottom), _
     New Direct3D.CustomVertex.PositionColored(-10, -10, -1, m_ColourBottom), _
     New Direct3D.CustomVertex.PositionColored(10, 0, -1, m_ColourBottom), _
     New Direct3D.CustomVertex.PositionColored(10, -10, -1, m_ColourBottom) _
    }
 ...
 Private Sub HODBGTexGen_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
  ' Stop rendering.
