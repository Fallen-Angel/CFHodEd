...
''' </typeparam>
Public Class GBasicMesh(Of VertexType As {Structure, IVertex}, _
 ...
 Public Function Add(Of InVertexType As {Structure, IVertex}, _
                    ...
                    (ByVal P As GMeshPart(Of InVertexType, InIndiceType, InMaterialType), _
            Optional ByVal _VC As Converter(Of InVertexType, VertexType) = Nothing, _
            Optional ByVal _IC As Converter(Of InIndiceType, IndiceType) = Nothing, _
 ...
 Public Function Append(Of InVertexType As {Structure, IVertex}, _
                           InIndiceType As {Structure, IConvertible}, _
                           InMaterialType As {Structure, IMaterial}) _
                       (ByVal obj As GBasicMesh(Of InVertexType, InIndiceType, InMaterialType), _
               Optional ByVal _VC As System.Converter(Of InVertexType, VertexType) = Nothing, _
               Optional ByVal _IC As System.Converter(Of InIndiceType, IndiceType) = Nothing, _
 ...
 Public Function CopyTo(Of OutVertexType As {Structure, IVertex}, _
                       ...
                       (ByVal obj As GBasicMesh(Of OutVertexType, OutIndiceType, OutMaterialType), _
 ...
 : Exit Function
  ...
  m_Parts.ForEach( _
   Function(V As GMeshPart(Of VertexType, IndiceType, MaterialType)) _
    (V.Transform(M, _VertexTransformer)) _
  )
 ...
 : Exit Function
  ...
  m_Parts.ForEach( _
   Function(_V As GMeshPart(Of VertexType, IndiceType, MaterialType)) _
    (_V.Translation(V, _VertexTransformer)) _
  )
 ...
 : Exit Function
  ...
  m_Parts.ForEach( _
   Function(_V As GMeshPart(Of VertexType, IndiceType, MaterialType)) _
    (_V.Rotation(V, _VertexTransformer)) _
  )
 ...
 : Exit Function
  ...
  m_Parts.ForEach( _
   Function(_V As GMeshPart(Of VertexType, IndiceType, MaterialType)) _
    (_V.Scaling(V, _VertexTransformer)) _
  )
 ...
 Public Function Lock(ByVal _Device As Device) As Boolean
  ...
  Lock( _
       _Device, _
       LockFlags.None, _
       New VertexBuffer(GetType(VertexType), _
                        TotalVertexCount, _
                        _Device, _
                        Usage.WriteOnly, _
                        Vtx.Format, _
       ...
       0, _
       New IndexBuffer(_Device, _
                       IndSize * TotalIndiceCount, _
                       Usage.WriteOnly, _
                       Pool.Managed, _
                       CBool(IndSize <> 4) _
                      ), _
 ...
 ''' </exception>
 Friend Function Lock(ByVal _Device As Device, _
                      ByVal _LockFlags As LockFlags, _
                      ByVal VB As VertexBuffer, _
                      ByVal BaseVertexOffset As Integer, _
                      ByVal IB As IndexBuffer, _
 ...
 : Exit Function
  ...
  For I As Integer = 0 To m_Parts.Count - 1
   With m_Parts(I)
    ...
    OffsetV += .Vertices.WriteDataToBuffer( _
                                           m_VB, _
                                           BaseVertexOffset * Vtx.VertexSize + OffsetV, _
                                           _LockFlags _
                                          )
    ...
    For J As Integer = 0 To m_Parts(I).PrimitiveGroupCount - 1
     ...
     ' Write the indices.
     OffsetI += .PrimitiveGroups(J).WriteDataToBuffer( _
                                                      m_IB, _
                                                      BaseIndiceOffset * BufIndSize + OffsetI, _
                                                      _LockFlags _
                                                      )
 ...
 : Exit Function
  ...
  For I As Integer = 0 To m_Parts.Count - 1
   ' Render the part.
   Dim _IndRendered As Integer = _
 _
   m_Parts(I).Render( _
                     _Device, m_VB, m_IB, _
                     m_BaseVertexOffset + VOffset, _
                     m_BaseIndiceOffset + IOffset _
                    )
