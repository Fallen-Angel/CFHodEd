...
Namespace Standard
 ...
 Public Structure Vertex
  ...
  ''' Diffuse colour.
  ''' </param>
  Public Sub New(ByVal _Position As Vector3, _
                 ByVal _Normal As Vector3, _
                 ByVal _TexCoords As Vector2, _
  ...
  Public ReadOnly Property Format() As VertexFormats Implements IVertex.Format
   Get
    Return _
     VertexFormats.Position Or _
     VertexFormats.Normal Or _
     VertexFormats.Diffuse Or _
  ...
  Public Shared Operator =(ByVal L As Vertex, ByVal R As Vertex) As Boolean
   If (L.Position = R.Position) AndAlso _
      (L.Normal = R.Normal) AndAlso _
      (L.DiffuseARGB = R.DiffuseARGB) AndAlso _
  ...
  ''' </returns>
  Public Shared Operator <>(ByVal L As Vertex, ByVal R As Vertex) As Boolean
   If (L.Position <> R.Position) OrElse _
      (L.Normal <> R.Normal) OrElse _
      (L.DiffuseARGB <> R.DiffuseARGB) OrElse _
      (L.TexCoords <> R.TexCoords) Then _
    Return True _
   Else _
    Return False
  ...
  ''' The modified vertex.
  ''' </returns>
  Private Function SetPosition3(ByVal V As Vector3) As IVertex Implements VertexFields.IVertexPosition3.SetPosition3
   Position = V
   Return Me
  ...
  ''' </remarks>
  Private Function GetTexCoords2(Optional ByVal Index As Integer = 0) As Vector2 Implements VertexFields.IVertexTex2.GetTexCoords2
   Return TexCoords
  ...
  ''' All index values point to the same set.
  ''' </remarks>
  Private Function SetTexCoords2(ByVal V As Vector2, Optional ByVal Index As Integer = 0) As IVertex Implements VertexFields.IVertexTex2.SetTexCoords2
   TexCoords = V
   Return Me
