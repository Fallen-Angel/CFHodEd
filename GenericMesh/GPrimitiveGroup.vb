...
Public NotInheritable Class GPrimitiveGroup(Of IndiceType As {Structure, IConvertible})
 ...
 Public Function Append(Of InIndiceType As {Structure, IConvertible}) _
                       (ByVal IndiceData() As InIndiceType, _
 ...
 ''' </remarks>
 Public Function Append(Of InIndiceType As {Structure, IConvertible}) _
                       (ByVal obj As GPrimitiveGroup(Of InIndiceType), _
               Optional ByVal _Converter As System.Converter(Of InIndiceType, IndiceType) = Nothing) As Boolean
 ...
 : Exit Function
  ...
  If Me.IndicesPerPrimitive <> obj.IndicesPerPrimitive Then _
   Throw New MeshPrimitiveTypeException("'IndicesPerPrimitive' not same for current primitive " & _
                                        "group and input primitive group.") _
 : Exit Function

  ' Convert this instance to "list" type if needed.
  If (Me.m_Type = PrimitiveType.LineStrip) OrElse _
     (Me.m_Type = PrimitiveType.TriangleStrip) OrElse _
  ...
  If (obj.m_Type = PrimitiveType.LineStrip) OrElse _
     (obj.m_Type = PrimitiveType.TriangleStrip) OrElse _
 ...
 : Exit Function
  ...
  If GetType(OutIndiceType) Is GetType(IndiceType) Then
   ' Copy directly.
   ' --------------
   m_Indices.CopyTo(SourceIndex, _
                    CType(CObj(P), IndiceType()), _
                    DestinationIndex, _
                    Length)
 ...
 ''' </remarks>
 Public Function CopyTo(Of OutIndiceType As {Structure, IConvertible}) _
                       (ByVal obj As GPrimitiveGroup(Of OutIndiceType), _
               Optional ByVal _Converter As System.Converter(Of IndiceType, OutIndiceType) = Nothing) As Boolean
 ...
 Public Function ConvertToList() As Boolean
  ...
  If (m_Type = PrimitiveType.TriangleList) OrElse _
     (m_Type = PrimitiveType.LineList) OrElse _
 ...
 Public Function ReverseFaceOrder() As Boolean
  ...
  If (m_Type = PrimitiveType.PointList) OrElse _
     (m_Type = PrimitiveType.LineList) OrElse _
 ...
 ''' </exception>
 Friend Function WriteDataToBuffer(ByVal IB As IndexBuffer, ByVal LockAtOffsetInBytes As Integer, ByVal _LockFlags As LockFlags) As Integer
 ...
 Private Function WriteDataToBuffer(Of OutIndiceType As {Structure, IConvertible}) _
                                   (ByVal IB As IndexBuffer, _
                                    ByVal LockAtOffsetInBytes As Integer, _
