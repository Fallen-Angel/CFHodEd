...
Public NotInheritable Class IFFReader
    ...
    ''' </param>
    Public Sub [AddHandler](ByVal ID As String, ByVal Type As ChunkType, ByVal Handler As ChunkHandler,
    ...
    ''' </returns>
    Friend Function FindHandler(ByVal ID As String, ByVal Type As ChunkType,
    ...
    Private Shared Function __swap(ByVal v As UInteger) As UInteger
        Return ((CUInt((v And &HFF000000) >> 24)) Or
                (CUInt((v And &HFF0000) >> 16) << 8) Or
                (CUInt((v And &HFF00) >> 8) << 16) Or
                (CUInt((v And &HFF)) << 24))
    End Function
    ...
    Private Shared Sub __defaultChunkHandler(ByVal Reader As IFFReader, ByVal ChunkAttributes As ChunkAttributes)
        Trace.TraceInformation("Skipping chunk:" & vbCrLf &
                               " ID: """ & ChunkAttributes.ID & """" & vbCrLf &
                               " Type: '" & ChunkAttributes.Type.ToString() & "'" & vbCrLf &
                               CStr(IIf(ChunkAttributes.Type = ChunkType.Normal,
                                        " Version: " & CStr(ChunkAttributes.Version), "")))
    End Sub
End Class
