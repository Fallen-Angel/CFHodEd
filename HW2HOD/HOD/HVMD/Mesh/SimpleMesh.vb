...
Public NotInheritable Class SimpleMesh
 ...
 Friend Sub WriteIFF(ByVal IFF As IFF.IFFWriter)
  If (Me.PartCount = 0) OrElse _
     (Me.TotalVertexCount = 0) OrElse _
 ...
 Private Sub PreWrite()
  ...
  With Me.Part(0)
   For I As Integer = .PrimitiveGroupCount - 1 To 0 Step -1
    If ((m_IsWireframe) AndAlso (.PrimitiveGroups(I).Type <> Direct3D.PrimitiveType.LineList)) OrElse _
       ((Not m_IsWireframe) AndAlso (.PrimitiveGroups(I).Type <> Direct3D.PrimitiveType.TriangleList)) Then _
     Trace.TraceWarning("Primitive in simple mesh removed because it doesn't have the proper primitive type.") _
