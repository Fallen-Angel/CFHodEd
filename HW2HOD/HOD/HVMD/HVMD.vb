...
Partial Class HOD
 ...
 Private Sub WriteHVMDChunk(ByVal IFF As IFF.IFFWriter)
  ...
  If (m_Name = Name_MultiMesh) OrElse _
 ...
 : WriteVARYChunk(IFF)
  ...
  If (m_Name = Name_SimpleMesh) OrElse _
 ...
 Private Sub WriteGOBGChunk(ByVal IFF As IFF.IFFWriter)
  For I As Integer = 0 To m_GoblinMeshes.Count - 1
   ...
   If m_RootJoint.GetJointByName(m_GoblinMeshes(I).ParentName) Is Nothing Then _
    Trace.TraceWarning("The joint referenced by goblin mesh " & CStr(I) & " does not exist, changed to root.") _
 ...
 Private Sub RenderHVMD(ByVal Device As Direct3D.Device, ByVal Transform As Matrix)
  ...
  Dim oldLighting As Boolean = Device.RenderState.Lighting, _
      oldSpecularEnable As Boolean = Device.RenderState.Lighting, _
