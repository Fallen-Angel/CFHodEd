Partial Class HOD
 ...
 Public Sub Rotation(ByVal v As Vector3)
  Dim m As Matrix = Matrix.RotationX(v.X) * _
  ...
  For I As Integer = 0 To m_Dockpaths.Count - 1
   For J As Integer = 0 To m_Dockpaths(I).Dockpoints.Count - 1
    m_Dockpaths(I).Dockpoints(J).Position = Vector3.TransformCoordinate(m_Dockpaths(I).Dockpoints(J).Position, m)
 ...
 Public Sub Mirroring(ByVal v As Vector3)
  ...
  Dim scale As New Vector3(CSng(IIf(v.X <> 0, -1, 1)), _
                           CSng(IIf(v.Y <> 0, -1, 1)), _
                           CSng(IIf(v.Z <> 0, -1, 1)))
