...
Public NotInheritable Class Dockpath
 ...
 ''' </summary>
 Friend Sub Render(ByVal Device As Direct3D.Device, ByVal Transform As Matrix, _
          Optional ByVal DockpointMesh As Standard.BasicMesh = Nothing, _
          Optional ByVal DockpointToleranceMesh As Standard.BasicMesh = Nothing, _
  ...
  ' Cache some states.
  Dim oldLighting As Boolean = Device.RenderState.Lighting, _
      oldSpecularEnable As Boolean = Device.RenderState.SpecularEnable, _
      oldAmbient As Drawing.Color = Device.RenderState.Ambient, _
      oldViewport As Direct3D.Viewport = Device.Viewport, _
  ...
  If DockpointMesh IsNot Nothing Then
   For I As Integer = 0 To m_Points.Count - 1
    ...
    If DockpointMesh IsNot Nothing Then
     ' Set world transform.
     Device.Transform.World = Matrix.Scaling(DockpointScale, DockpointScale, DockpointScale) * _
                              Matrix.RotationX(p.Rotation.X) * _
                              Matrix.RotationY(p.Rotation.Y) * _
                              Matrix.RotationZ(p.Rotation.Z) * _
                              Matrix.Translation(p.Position) * _
                              Transform
    ...
    If (DockpointToleranceMesh IsNot Nothing) OrElse (p.PointTolerance < 0.001) Then
     ...
     ' Set world transform.
     Device.Transform.World = Matrix.Scaling(p.PointTolerance, p.PointTolerance, p.PointTolerance) * _
                              Matrix.Translation(p.Position) * _
                              Transform
