...
Public NotInheritable Class CollisionMesh
 ...
 Private Sub PrepareMeshForExport()
  ...
  With Me.Part(0)
   For I As Integer = 0 To .PrimitiveGroups(0).IndiceCount - 3 Step 3
    ' Get indices.
    Dim ind1 As Integer = .PrimitiveGroups(0).Indice(I), _
        ind2 As Integer = .PrimitiveGroups(0).Indice(I + 1), _
        ind3 As Integer = .PrimitiveGroups(0).Indice(I + 2)

    ' Get vertices.
    Dim v1 As PNVertex = .Vertices(ind1), _
        v2 As PNVertex = .Vertices(ind2), _
    ...
    ' Calculate area.
    Dim area As Single = 0.5F * Math.Abs(Vector3.Cross(v2.Position - v1.Position, _
 ...
 Friend Sub RenderBox(ByVal Device As Direct3D.Device, ByVal Transform As Matrix)
  ...
  Dim v() As Direct3D.CustomVertex.PositionColored = { _
         New Direct3D.CustomVertex.PositionColored(m_Min.X, m_Min.Y, m_Min.Z, c), _
         New Direct3D.CustomVertex.PositionColored(m_Max.X, m_Min.Y, m_Min.Z, c), _
         New Direct3D.CustomVertex.PositionColored(m_Max.X, m_Min.Y, m_Max.Z, c), _
         New Direct3D.CustomVertex.PositionColored(m_Min.X, m_Min.Y, m_Max.Z, c), _
         New Direct3D.CustomVertex.PositionColored(m_Min.X, m_Max.Y, m_Min.Z, c), _
         New Direct3D.CustomVertex.PositionColored(m_Max.X, m_Max.Y, m_Min.Z, c), _
         New Direct3D.CustomVertex.PositionColored(m_Max.X, m_Max.Y, m_Max.Z, c), _
         New Direct3D.CustomVertex.PositionColored(m_Min.X, m_Max.Y, m_Max.Z, c)}

  ' Prepare vertices to be rendered.
  Dim vtx() As Direct3D.CustomVertex.PositionColored = { _
   v(0), v(1), v(1), v(2), v(2), v(3), v(3), v(0), _
   v(4), v(5), v(5), v(6), v(6), v(7), v(7), v(4), _
   v(0), v(4), v(1), v(5), v(2), v(6), v(3), v(7)}

  ' Cache some states.
  Dim oldAmbient As Integer = Device.RenderState.AmbientColor, _
 ...
 ''' </summary>
 Friend Sub RenderSphere(ByVal Device As Direct3D.Device, ByVal Sphere As Standard.BasicMesh, ByVal Transform As Matrix)
  If Not m_Visible Then _
   Exit Sub

  Dim oldAmbient As Integer = Device.RenderState.AmbientColor, _
      oldCullMode As Direct3D.Cull = Device.RenderState.CullMode, _
      oldFillMode As Direct3D.FillMode = Device.RenderState.FillMode, _
      oldLighting As Boolean = Device.RenderState.Lighting
  ...
  Device.RenderState.Lighting = False
  Device.Transform.World = Matrix.Scaling(m_Radius, m_Radius, m_Radius) * _
                           Matrix.Translation(m_Center) * _
                           Transform
