...
Friend NotInheritable Class BSRM
 ...
 ''' </summary>
 Private Shared Sub WriteDesc(ByVal IFF As IFF.IFFWriter, _
                              ByVal name As String, ByVal parentName As String, _
    ...
    For X As Integer = 0 To xDiv - 1
     ...
     With bm.Part(0)
      For I As Integer = 0 To .PrimitiveGroups(0).IndiceCount - 3 Step 3
       ...
       ' Get vertices.
       Dim v1 As Vector3 = .Vertices(ind1).Position, _
           v2 As Vector3 = .Vertices(ind2).Position, _
           v3 As Vector3 = .Vertices(ind3).Position

       ' See if it lies in cell.
       If (cellMin.X <= v1.X) AndAlso (cellMin.Y <= v1.Y) AndAlso (cellMin.Z <= v1.Z) AndAlso _
          (cellMax.X >= v1.X) AndAlso (cellMax.Y >= v1.Y) AndAlso (cellMax.Z >= v1.Z) Then _
        l.Add(CUShort(I \ 3)) _
      : Continue For

       ' See if it lies in cell.
       If (cellMin.X <= v2.X) AndAlso (cellMin.Y <= v2.Y) AndAlso (cellMin.Z <= v2.Z) AndAlso _
          (cellMax.X >= v2.X) AndAlso (cellMax.Y >= v2.Y) AndAlso (cellMax.Z >= v2.Z) Then _
        l.Add(CUShort(I \ 3)) _
      : Continue For

       ' See if it lies in cell.
       If (cellMin.X <= v3.X) AndAlso (cellMin.Y <= v3.Y) AndAlso (cellMin.Z <= v3.Z) AndAlso _
      ...
      : Continue For
       ...
       ' See if it lies in cell.
       If (cellMin.X <= v.X) AndAlso (cellMin.Y <= v.Y) AndAlso (cellMin.Z <= v.Z) AndAlso _
 ...
 Friend Shared Sub WriteGoblinMesh(ByVal IFF As IFF.IFFWriter, ByVal gm As GoblinMesh)
  If (gm.Mesh.PartCount = 0) OrElse _
     (gm.Mesh.TotalVertexCount = 0) OrElse _
 ...
 : Exit Sub
  ...
  If (mm.LOD(0).PartCount = 0) OrElse _
     (mm.LOD(0).TotalVertexCount = 0) OrElse _
